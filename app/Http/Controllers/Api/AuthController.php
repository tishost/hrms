...
class AuthController extends Controller
{
    ...
    private function normalizeProfilePic(?string $path): ?string
    {
        ...
        // Always append current timestamp for cache busting
        $appendVersion = function (string $url) {
            $timestamp = time();
    ...
    {
        // Check for soft deleted accounts with same email
        ...
        if ($softDeletedUser) {
            return response()->json([
                ...
                'deleted_at' => $softDeletedUser->deleted_at->format('Y-m-d H:i:s')
            ], 409);
        }
    ...
    public function registerOwner(OwnerRegistrationRequest $request)
    {
        try {
            ...
            // Check for soft deleted accounts with same email or phone
            $softDeletedUser = User::onlyTrashed()->where('email', $request->email)->first();
            $softDeletedOwner = Owner::onlyTrashed()->where('email', $request->email)->orWhere('phone', $request->phone)->first();
            
            if ($softDeletedUser || $softDeletedOwner) {
                return response()->json([
                    'success' => false,
                    'message' => 'An account with this email or phone already exists but is deactivated.',
                    'restore_available' => true,
                    'restore_message' => 'Do you want to restore your old account? If yes, your account will be reactivated.',
                    'deleted_at' => $softDeletedUser ? $softDeletedUser->deleted_at->format('Y-m-d H:i:s') : $softDeletedOwner->deleted_at->format('Y-m-d H:i:s')
                ], 409);
            }
